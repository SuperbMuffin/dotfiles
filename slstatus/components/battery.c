	...
	battery_perc(const char *bat)
	{
		...
		if (pscanf(path, "%d", &cap_perc) != 1)
			return NULL;
                ...
                snprintf(perc_str, sizeof(perc_str), "%d", cap_perc);

                // get the battery icon
                icon = get_icon(perc_str, 'b');
                
                // combine icon + percentage
                snprintf(buf, sizeof(buf), "%s %s%%", icon, perc_str);
